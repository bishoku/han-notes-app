      ...
      <div className="flex items-center p-2 border-b border-mac-borderLight dark:border-mac-borderDark gap-1 shrink-0">
        <button 
          ...
          className={cn(
            ...
            activeTab === 'links' ? "bg-white dark:bg-zinc-800 shadow-mac-panel text-mac-accent font-medium" : "text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
          )}
          title={t('backlinks')}
        >
          <Link2 size={16} />
        ...
        <button 
          ...
          className={cn(
            "flex-1 flex justify-center py-1.5 rounded-md transition-all duration-150 relative",
            activeTab === 'outline' ? "bg-white dark:bg-zinc-800 shadow-mac-panel text-gray-900 dark:text-gray-100" : "text-gray-500 hover:text-gray-700 dark:hover:text-gray-300"
          )}
          title={t('outline')}
        >
          <List size={16} />
        ...
        {activeTab === 'links' && (
          <div>
            <div className="flex items-center justify-between mb-4">
              <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('backlinks')}</div>
          ...
          <div className="flex flex-col h-full gap-3 overflow-hidden">
            ...
            <div className="flex-1 overflow-y-auto min-h-[140px] pr-1">
              <div className="flex items-center justify-between mb-2 sticky top-0 bg-mac-sidebarLight dark:bg-mac-sidebarDark py-1 z-10">
                <div className="text-xs font-semibold text-gray-400 uppercase tracking-wider">{t('outline')}</div>
              ...
              {noteTasks.length === 0 ? (
                ...
                <div className="flex flex-col gap-2">
                  {noteTasks.map((task, idx) => {
                    ...
                    const overdue = !task.completed && task.end_date && task.end_date < todayStr && (task.progress ?? 0) < 100;
                    ...
                    return (
                      ...
                      >
                        <div className="flex items-start justify-between gap-2">
                          <div className="flex items-start gap-2 flex-1 min-w-0">
                            ...
                            <span className={cn(
                              ...
                              (task.completed || (task.progress ?? 0) === 100) && "line-through text-gray-400 dark:text-gray-500"
                            )}>
                              {task.content}
                            </span>
                          </div>
                          ...
                          <button
                            ...
                            className="p-1 rounded-md hover:bg-black/5 dark:hover:bg-white/5 text-gray-400 hover:text-mac-accent transition-colors shrink-0"
                        ...
                        {/* Task Description (Açıklama Kısmı) */}
                        {task.description && (
                          ...
                          {assigneesList.length > 0 && (
                            <div className="flex items-center gap-1">
                              {assigneesList.map((person) => (
                                <span key={person} className="text-blue-600 dark:text-blue-400 flex items-center gap-0.5 bg-blue-500/10 px-1.5 py-0.5 rounded font-medium border border-blue-500/20">
                                  <User size={10} /> {person}
                                </span>
                              ))}
                            </div>
